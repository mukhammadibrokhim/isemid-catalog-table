<template>
  ...
  export default {
    ...
    data() {
      return {
        localEditedCatalog: {}, // Boshida bo'sh obyekt yaratamiz
        typeList: [],
      };
    },
    watch: {
      // editedCatalog o'zgarishi bilan uni localEditedCatalog ga nusxalab olamiz
      editedCatalog: {
        handler(newVal) {
          this.localEditedCatalog = { ...newVal };
        },
        immediate: true, // Komponent yuklanganda ham ishlaydi
        deep: true, // Ichki obyektlar o'zgarishini ham kuzatadi
      },
    },
    methods: {
      async fetchTypes() {
      ...
      saveChanges() {
        this.$emit("save", { ...this.localEditedCatalog });
      },
      closeModal() {
        this.$emit("close");
      },
